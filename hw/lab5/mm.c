...
*/
#define SIZE(x) ((x) & ~(ALIGNMENT - 1)) // e.g. ignores all the tag bits
...
// Note all this does is adjust the prev/next pointers, not the headers/footers
...
void* mm_malloc(size_t size) {
	...
	ptrFreeBlock = searchFreeList(reqSize); // traverse explicit free list for possible free block

	if (ptrFreeBlock == NULL) { // If no free blocks exist, allocate more space on heap
		//ptrFreeBlock = (BlockInfo*) UNSCALED_POINTER_ADD(mem_heap_hi(), 1); // get the address of spot right after end of the heap (mem_heap_hi gives address of last byte of heap, so move one byte forward)
		requestMoreSpace(reqSize); // request enough space for our new block; this will append a block to the free list
	...
	// Check if the preceding block is used or not, and check the current block's size
...
void mm_free(void *ptr) {
  ...
  // NOTE ptr points to the start of the payload, NOT the start of the block (e.g. the header)
  // So we first get a pointer to the start of the block
  ...
  // Set header USED tag to zero via a bitwise XOR
  blockInfo->sizeAndTags &= (~TAG_USED);
  
  // Copy header to footer
  size_t* footer = (size_t*) UNSCALED_POINTER_SUB(followingBlock, WORD_SIZE);
  *footer = blockInfo->sizeAndTags;

  // Adjust header of the next block and set PRECEDING_USED tag to zero
  followingBlock->sizeAndTags &= (~TAG_PRECEDING_USED);

  // check if following block is used and adjust footer too if not?
