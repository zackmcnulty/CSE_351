...
void part2() {
    ...
    //
    // Zack: C will run the code anyways, you just haven't allocated memory for this
    fillArray(array, 10);
    ...
    printf("value: %i\n", value);
    // the value printed is 2. We know at the address &value there is stored a 4byte/8 byte integer (namely, the integer value=351)
	// But memory alone cannot differentiate between a single integer and just the first element of an integer array. FillArray reassigns the
	// first element of an integer array starting at &value the value of 0*3 + 2 = 2.
}
...
// Z: Seems to behave similarly to a java class with no methods.
...
void part5() {
    ...
    //free(class_grades);
	//
	// Z: upon commenting out the free statement, valgrind mentions there were 80 bytes of unfreed
	//    memory upon the conclusion of the program.
}
...
int main(int argc, char* argv[]) {
    ...
    // Namely, argv[0] = "lab0.c"

    // If you didn't pass an argument or if that argument was zero
